...
de_analysis <- function(dge) {
  # function to do different expression analysis with DGE object and output de genes to reports/de_genes
  # Args:
  ...
  contrasts <- makeContrasts(
    time_24_vs_0_rnaseq = time_24_rnaseq - time_0_rnaseq,
    time_24_vs_0_riboseq = time_24_riboseq - time_0_riboseq,
    time_24_vs_0_te = (time_24_riboseq - time_0_riboseq) - (time_24_rnaseq - time_0_rnaseq),
    levels = design
  )
  ...
  saveRDS(de, file.path(OUTPUT, "objs", "de.rds"))
  de_results_path <- file.path(OUTPUT, "reports", "de_genes")
  for (comp in attr(contrasts, 'dimnames')$Contrasts) {
    write_csv(filter(de[[comp]], FDR < .1), file.path(de_results_path, paste0(comp, ".csv")))
  }
